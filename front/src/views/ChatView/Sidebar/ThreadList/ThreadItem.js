...
function contactSelector(state, thread, userId) {
  ...
  const contactId = thread.participantIds.filter(
...
function ThreadItem({ active, thread, className, ...rest }) {
  const classes = useStyles();
  const { user } = useSelector((state) => state.account);
  const contact = useSelector((state) =>
    contactSelector(state, thread, user.id)
  );

  const lastMessage = thread.messages[thread.messages.length - 1];
  const lastMessageInfo = lastMessage
    ? `${lastMessage.senderId === user.id ? "Me:" : ""} ${
