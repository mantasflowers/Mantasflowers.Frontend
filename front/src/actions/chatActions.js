...
export function addMessageVanish({ userId, threadKey, body, attachments }) {
  const request = axios.post("/api/chat/messages/new", {
    userId,
    threadKey,
    body,
  ...
  return (dispatch) => {
    request.then((response) => {
      ...
      let threadKey = response.data.threadKey;
      let messageId = response.data.message.id;

      setTimeout(function () {
        const request = axios.post("/api/chat/messages/delete", {
          threadKey,
          messageId,
        });
